...
- **`b515bf6`/`2f061bd` CI vuln gate, `7c1c3e4` CI cache** — worth taking, deliberately not in this batch. If ported, take the **binary-mode** gate: source mode peaks at 4.3 GB and OOM-kills a 7 GB runner. Blocked on something else anyway — see the memory note that pushes to `main` have dispatched no Actions run since 2026-08-06.
...
5. **Distribution — half built (2026-07-17), half blocked on things only a human can do.**
   - **Done and testable now:** `pocket-ap version` (ldflags-stamped, goreleaser's default var names), `make dist` (cross-compiles all 5 targets), `.goreleaser.yaml`, `Dockerfile`. Verify with `goreleaser release --snapshot --clean` — no remote or tag needed.
   - **UNBLOCKED 2026-08-19 — brew is ready to fire, npm is genuinely unbuilt.** This line spent weeks saying "no git remote", then "the remote is PRIVATE". Both are now history: the repo is **public**, the history was squashed to a single root commit first, and `pokt-network/homebrew-tap` exists (public, empty, verified to `brew tap` cleanly — it holds one `Formula/<name>.rb` per project, which is why it is `homebrew-tap` and not `homebrew-pocket-ap`).
     - **A `v*` tag now triggers a real release.** `.github/workflows/release.yml` runs goreleaser: five binaries, GitHub release, formula push, ghcr images. Tag deliberately.
     - **The tap needs its own credential** — `HOMEBREW_TAP_GITHUB_TOKEN`, an org secret shared with this repo. `GITHUB_TOKEN` is scoped to this repository and cannot write to the tap. The workflow's `preflight` job checks `.permissions.push` **before** anything publishes, because goreleaser pushes the formula *after* the release is out, and discovering a bad token there is the worst possible moment.
     - **npm has NO scaffolding at all** — no `package.json`, and goreleaser has no npm publisher, so this is a wrapper someone has to write. Both `pocket-ap` and `@pokt-network/pocket-ap` are free on the registry. At ~102 MB stripped per platform the sane shape is a postinstall that downloads the matching archive from the GitHub release **and verifies it against `checksums.txt`** — without that an npm install trusts the network completely.
     - LICENSE is done (MIT, matching poktroll/PATH/relay-miner).
   - **The repo being destined for public is a live constraint on what gets written here, not a future one.** Anything committed now is what ships the moment visibility flips — so no third-party names in tracked files (attribute neutrally or link the public issue), and no key material, ever. `TestNoSecretsInTrackedFiles` is the backstop, not the policy.
