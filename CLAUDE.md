...
5. **Distribution — half built (2026-07-17), half blocked on things only a human can do.**
   ...
   - **⚠️ BLOCKED, not unbuilt — but the blocker moved (checked 2026-08-04).** The remote now exists: `origin` → `https://github.com/pokt-network/pocket-ap.git`, created 2026-07-23, **and it is PRIVATE**. So "there is no git remote", which this line said for weeks, is obsolete; what actually blocks brew and npm is that both fetch from a **public** URL, so the repo has to go public *and* carry a tagged release. Still also needs an **npm org** and a **pokt-network/homebrew-tap** repo. (LICENSE is done — MIT, matching poktroll/PATH/relay-miner.)
